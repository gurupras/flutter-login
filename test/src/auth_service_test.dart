...
void main() {
  ...
  group('AuthService', () {
    ...
    tearDown(() {
      TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
          .setMockMethodCallHandler(
              const MethodChannel('me.gurupras.liblogin'), null);
      TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
          .setMockMethodCallHandler(
    ...
    test('init generates and stores device ID if not present', () async {
      when(mockSecureStorage.read(key: 'deviceID')).thenAnswer((_) async => null);
      when(mockSecureStorage.write(key: 'deviceID', value: anyNamed('value')))
          .thenAnswer((_) async => {});
    ...
    test('login calls FusionAuthClient and stores tokens on success', () async {
      ...
      when(mockFusionAuthClient.resourceOwnerPasswordCredentialsGrant(any, any))
      ...
      verify(mockSecureStorage.write(key: 'refreshToken', value: 'refresh')).called(1);
      verify(mockSecureStorage.write(key: 'userID', value: 'user123')).called(1);
    });
    ...
    test('signUp calls http client and then login on success', () async {
      ...
      verify(mockFusionAuthClient.resourceOwnerPasswordCredentialsGrant('newuser', 'newpass'))
          .called(1);
    });
    ...
    test('signUp returns false if http client post fails', () async {
      ...
      verify(mockHttpClient.post(any, headers: anyNamed('headers'), body: anyNamed('body')))
          .called(1);
      verifyNever(mockFusionAuthClient.resourceOwnerPasswordCredentialsGrant(any, any));
    });
    ...
    group('checkLoginStatus', () {
      test('returns true if access token is valid and not expired', () async {
        when(mockSecureStorage.read(key: 'accessToken')).thenAnswer((_) async => 'valid_access_token');
        when(mockSecureStorage.read(key: 'refreshToken')).thenAnswer((_) async => 'valid_refresh_token');
        when(mockSecureStorage.read(key: 'userID')).thenAnswer((_) async => 'user123');
      ...
      test('returns true if access token is expired but refresh token is valid and refreshes', () async {
        when(mockSecureStorage.read(key: 'accessToken')).thenAnswer((_) async => 'expired_access_token');
        when(mockSecureStorage.read(key: 'refreshToken')).thenAnswer((_) async => 'valid_refresh_token');
        when(mockSecureStorage.read(key: 'userID')).thenAnswer((_) async => 'user123');
        when(mockJwtDecoder.isExpired('expired_access_token')).thenReturn(true);
        ...
        when(mockFusionAuthClient.refreshTokenGrant('valid_refresh_token'))
        ...
        verify(mockFusionAuthClient.refreshTokenGrant('valid_refresh_token')).called(1);
        verify(mockSecureStorage.write(key: 'accessToken', value: 'new_access')).called(1);
        verify(mockSecureStorage.write(key: 'refreshToken', value: 'new_refresh')).called(1);
      });

      test('returns false if both access and refresh tokens are invalid', () async {
        when(mockSecureStorage.read(key: 'accessToken')).thenAnswer((_) async => 'expired_access_token');
        when(mockSecureStorage.read(key: 'refreshToken')).thenAnswer((_) async => 'invalid_refresh_token');
        when(mockSecureStorage.read(key: 'userID')).thenAnswer((_) async => 'user123');
        when(mockJwtDecoder.isExpired('expired_access_token')).thenReturn(true);
        when(mockFusionAuthClient.refreshTokenGrant('invalid_refresh_token'))
        ...
        verify(mockSecureStorage.read(key: 'accessToken')).called(1);
        verify(mockJwtDecoder.isExpired('expired_access_token')).called(1);
        verify(mockFusionAuthClient.refreshTokenGrant('invalid_refresh_token')).called(1);
        verify(mockSecureStorage.delete(key: 'accessToken')).called(1);
        verify(mockSecureStorage.delete(key: 'refreshToken')).called(1);
        verify(mockSecureStorage.delete(key: 'userID')).called(1);
      });
    ...
    group('initiateGoogleLogin', () {
      test('initiates Google login successfully', () async {
        when(mockSecureStorage.write(key: 'code_verifier', value: anyNamed('value')))
            .thenAnswer((_) async => {});
    ...
    group('_processAuthRedirect', () {
      test('processes redirect with code successfully', () async {
        ...
        when(mockFusionAuthClient.exchangeAuthorizationCode('auth_code', 'mock_code_verifier'))
            .thenAnswer((_) async => tokenResponse);
        when(mockSecureStorage.write(key: anyNamed('key'), value: anyNamed('value')))
            .thenAnswer((_) async => {});
      ...
      test('emits false on authRedirectStream if token exchange fails', () async {
        final uri = Uri.parse('https://example.com/callback?code=auth_code');
